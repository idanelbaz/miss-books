...
import googleBooksUtil from './google-books-apiUtil.js'
...
function deleteReview(currReviewId, bookId) {
    gbooks.forEach(book => {
        if (book.id === bookId) {
            book.review.splice(currReviewId, 1);
        }
    })
...
function getById(bookId) {
    let currBook = query()
        .then(books => {
                const book = books.find(currBook => currBook.id === bookId);
...
function findBookIdx(book) {
    gbooks = storageService.load(BOOKS_KEY);
    let nextBook = gbooks.findIndex(bookPrev => {
        return bookPrev.id === book.id
    });
    nextBook += 1;
    return gbooks[nextBook].id;
}

function checkIfBookNew(book) {
    gbooks = storageService.load(BOOKS_KEY);
    if (gbooks.find(bookPrev => {
            return bookPrev.id === book.id
        })) return true;
    else return false;
}
...
function sendBookToPro(book) {
    if (gbooks.find(bookPrev => {
            return bookPrev.id === book.id
        })) return;

    else gbooks.push(book);
    storageService.store(BOOKS_KEY, gbooks);

}

function getGoogleBook(txt) {
    let googleBooks = googleBooksUtil.getGoogleBook(txt)
    var res = makeGoogleBooksSame(googleBooks)
        .then((res) => {
            return res
        })
...
function makeGoogleBooksSame(books) {

    let booksChange = books.then(newBooks => {
        return newBooks.items;
    })
    let booksToReturn = booksChange.then(books => {
        let newBooks = books.map(book => {
            book.review = [];
            book.title = book.volumeInfo.title;
            book.authors = book.volumeInfo.authors;
            book.description = book.volumeInfo.description;
            if (!book.description) book.description = book.title;
            book.language = book.volumeInfo.language;
            book.pageCount = book.volumeInfo.pageCount;
            book.publishedDate = book.volumeInfo.publishedDate;
            if (!book.volumeInfo.imageLinks) book.thumbnail = 'http://clipart-library.com/image_gallery/515071.png'
            else book.thumbnail = book.volumeInfo.imageLinks.thumbnail;
            if (!book.listPrice) book.listPrice = {};
            book.listPrice.isOnSale = book.saleInfo.saleability;
            if (!book.listPrice.isOnSale) book.listPrice.isOnSale = false;
            else if (book.listPrice.isOnSale = 'FREE') book.listPrice.isOnSale = true;
            else if (book.listPrice.isOnSale = 'NOT_FOR_SALE') book.listPrice.isOnSale = false;
            else if (book.listPrice.isOnSale = 'FOR_SALE') book.listPrice.isOnSale = true;
            if (!book.saleInfo.listPrice) book.listPrice.currency = 'Not on sale'
            else book.listPrice.currency = book.saleInfo.listPrice.currencyCode;
            if (!book.listPrice.currency) book.listPrice.currency = 'Not for Sale'
            if (!book.saleInfo.listPrice) book.listPrice.amount = 'Not for Sale';
            else book.listPrice.amount = book.saleInfo.listPrice.amount;
            return book;
